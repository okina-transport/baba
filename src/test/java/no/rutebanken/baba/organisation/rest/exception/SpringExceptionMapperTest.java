...
class SpringExceptionMapperTest {
	...
	void testMapValidationExceptionToBadRequest() {
        try (Response rsp = new SpringExceptionMapper().toResponse(new TransactionSystemException("", new ValidationException()))) {
	...
	void testMapUnknownExceptionToInternalServerError() {
        try (Response rsp = new SpringExceptionMapper().toResponse(new TransactionSystemException("", new RuntimeException()))) {
