...
class JpaProviderRepositoryTest extends BaseIntegrationTest {
    ...
    void testGetProviderById() {
        ...
        assertThat(provider).isEqualTo(new Provider(42L, "Flybussekspressen", "42", "test@test.com",
    ...
    void testCreateAndUpdateAndDeleteProvider() {
        ...
        Provider newProvider = new Provider(null, "junit provider", "sftpAccount", "mailAccount", chouetteInfo);
        ...
        assertEquals(providerForUpdate.sftpAccount, providerForVerification.sftpAccount);

        repository.deleteProvider(newProvider.id);

        Provider noProvider = repository.getProvider(newProvider.id);

        assertNull(noProvider);
    }
